...
{
    private $debug_query_time_in_ms = 0;
    /**
    ...
    public function boot()
    {
        ...
        Validator::extend('check_todo_unique_by_user_id', function ($attribute, $value, $parameters, $validator) {
            $user_id         = $parameters[0] ?? null;
            $todo_id    = $parameters[1] ?? null;
            $todo_count = Todo::getSimilarTodoByUserId($value, (int)$user_id, (int)$todo_id, true);
            return $todo_count == 0;
        });

        Validator::extend('check_todo_task_unique_by_todo_id', function ($attribute, $value, $parameters, $validator) {
            $todo_id         = $parameters[0] ?? null;
            $todo_task_id    = $parameters[1] ?? null;
            $todo_task_count = TodoTask::getSimilarTodoTaskByTodoId($value, (int)$todo_id, (int)$todo_task_id, true);
            return $todo_task_count == 0;
        });
...
function writeSqlToLog($contents, string $descr_text = '', bool $is_sql = false, string $file_name = '')
{
    ...
    try {
        if (empty($file_name)) {
            $file_name = storage_path() . '/logs/sql-tracing.log';
        }
...
function isDeveloperComp($check_debug = false)
{
    ...
    $url = config('app.url');
    $pos = strpos($url, 'local-todos.com');
    if ( ! ($pos === false)) {
        return true;
    }
